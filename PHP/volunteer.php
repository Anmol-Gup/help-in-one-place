<?php
    ...
    if(isset($_POST['submit'])){
        ...
        */
        ...
        other_occu, vol_dom_desc, vol_join_desc, resume) values (:volunteer_gender,:vol_dob,:vol_contact,:vol_address,:vol_city,:vol_state,:vol_occ,:other_occu,:vol_dom_desc,:vol_join_desc,:resume)";

        $stmt=$pdo->prepare($insertquery);
        $arr=[
            "volunteer_gender"=>$volunteer_gender,"vol_dob"=>$vol_dob,"vol_contact"=>$vol_contact,"vol_address"=>$vol_address,"vol_city"=>$vol_city,
            "vol_state"=>$vol_state,"vol_occ"=>$vol_occ,"other_occu"=>$other_occu,"vol_dom_desc"=>$vol_dom_desc,"vol_join_desc"=>$vol_join_desc,"resume"=>$destination
        ]; 
        $stmt->execute($arr);
